...
- **Progress tracking** — check off concepts, case studies, resources, questions, and plan weeks; an overall completion ring and per-module meters fill as you go.
- **Persistent** — everything saves to `localStorage`. Export your progress as JSON, or reset it.
- **Filterable & searchable** — filter case studies by domain, questions by tier, resources by category/cost; live search within every section.
- **Timed-drill mode** — a 🎲 button serves a random question to practice against a 35-minute clock.
- **Responsive** — works on desktop and mobile.

## Usage

Just open `index.html` in any modern browser — or use the [live site](https://iharshgarg04.github.io/system-design-portal/). No build step, no server, works offline. Your progress lives in the browser you use, so export the JSON if you want a backup or to move between machines.

## Tech

A single, dependency-free HTML file: embedded CSS and vanilla JavaScript, data-driven rendering from in-file arrays. Deliberately "boring technology" — it'll open identically years from now, no toolchain to rot.

## Sources

Content was compiled from primary engineering sources (company engineering blogs, foundational papers), interviewer-written guides, and community discussions (r/ExperiencedDevs, Hacker News), with key claims fact-checked against their originals. Every resource links out to its source; links open in a new tab.

---

*Built as a personal interview-prep hub. Resource links and trade-off framings reflect sources as of mid-2026 — verify specifics against the primary source before relying on them.*
